...
export default createTRPCRouter({
  ...
  create: loggerProcedure
    ...
    .mutation(async ({ input }) => {
      const [result] = await DB.insert(SCHEMA.TG.warnings).values(input).returning()
  ...
  getTotalActiveCount: loggerProcedure
    ...
    .query(async ({ input }) => {
      const [result] = await DB.select({ count: count() })
        ...
        .where(
          and(
            eq(SCHEMA.TG.warnings.targetId, input.targetId),
            isNull(SCHEMA.TG.warnings.deletedAt),
            eq(SCHEMA.TG.warnings.isExpired, false)
          )
        )
    ...
    .query(async ({ input }) => {
      ...
      const [result] = await DB.select({ count: count() })
        ...
        .where(
          and(
            eq(SCHEMA.TG.warnings.targetId, input.targetId),
            eq(SCHEMA.TG.warnings.groupId, input.groupId),
            isNull(SCHEMA.TG.warnings.deletedAt),
            eq(SCHEMA.TG.warnings.isExpired, false)
          )
        )
